...
var grid = Array.ofDim[Int](20,20)
//for(grid_i <- 0 until 20) {
//  for(grid_j <- 0 until 20){
//    grid(grid_i)(grid_j) = Random.nextInt(100)
//  }
//}
...
grid = strGid.split("\n").map(_.stripMargin.split(" ").map(_.toInt))

val i = 18
val diagIndex = 19 - i
val maxPerPosDiagonal = (0 to i).map(j => grid(j)(i - j))
val maxPerPosDiagonal2 = (0 to i).map(j => grid(diagIndex + j)(19 - j))
val maxPerNegDiagonal = (0 to i).map(j => grid(diagIndex + j)(j))
val maxPerNegDiagonal2 = (0 to i).map(j => grid(j)(diagIndex + j))
grid(16)(16)
...
//} yield grid(i)(j)
//for (i <- 0 until 20) {
//  //  println((0 until 20).map(grid(_)(i)).mkString(","))
//  println((0 until 20).map(grid(_)(i)).sliding(4).map(_.product).max)
//}
(0 until 20).map(i => {
  ...
  Seq(maxPerRow, maxPerCol, diagMax).max
}).max
